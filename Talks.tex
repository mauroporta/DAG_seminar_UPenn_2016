...
\end{tikzcd}
\]
This is a Cartesian fibration. Next:
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
$f$ is a Cartesian morphism. So we can apply Grothendieck construction to get a functor $\mathcal{F} : \cC Alg_k^{\op}
...
\begin{defin}
The \textbf{Cech nerve functor} $B^{\bullet} : \Delta \to \cC Alg_k$, $i \to B^{\otimes_A^{i+1}}$.
\end{defin}

\begin{rem}
We need to build this functor as a left Kan extension.
\end{rem}

The descent problem is: we get a map $A-Mod \to \varinjlim B^{\otimes_A^{n}} - Mod$, is this an equivalence? If it is,
construct some sort of inverse. The following lemma is 3.20 in Shennon - Porta - Vezzosi, Formal Gluing along non-linear
...
\begin{enumerate}
\item $\forall s \in S$ there is a t-structure $(\mathcal{X}_s^{\leq 0}, \mathcal{X}_s^{\geq 0})$on $\mathcal{X}_s$
\item $\forall $ edge $s \to s^1$, the induced functor $f^*$ is t-exact.
\end{enumerate}
...
\end{enumerate}
\end{lem}
Note that 1 implies 2 is obvious, while 2 implies 1 is super powerful. This is because we can do many constructions
at the level of the hearts that we can't do at the level of $\infty$-categories.

\begin{proof}
The first step is fully faithfulness. Fix $x,y \in \cC$ and get canonical transformation $\psi_{x,y} :
Map_{\cC}^{st}(x,y) \to Map_{\cD}^{st}(f(x), f(y))$. Start by fixing the full subcategory $\cC_x \subset \cC$,
spanned y those $y$ such that $\psi_{x,y}$ is an equivalence. This is closed under loop and suspension, extensions
and retract. So if $\cC^{\heartsuit} \subset \cC_X$, we go by induction on non-vanishing cohomology groups,
to get $\cC^b \subset \cC_X$. Here we use the left complete and right complete assumptions. $y = \varprojlim
\tau_{\geq n} y$, $f$ commutes with this specific colimit, so $f(y) = \varinjlim f(\tau_{\geq n} y)$.
...
Recall that we were trying to determine whether $A-Mod \to \varinjlim B^{\otimes_A^{n}} - Mod$ is an equivalence. We use
the first lemma to put a t-structure on the limit, and the second lemma to replace with descent for hearts. The RHS
becomes $\varinjlim (B^{\otimes_A^{n}} - Mod)^{\heartsuit} = \pi_0(B^{\otimes_A^{n}}) - Mod$.

Note that this is not quite the classical case, because the Cech nerve is infinite here. We'll need to apply
the following theorem for $\cC = \cC at \subset \cC at_{\infty}$, which is a 2-category. So we'll apply $n=2$.
...
Note that $J_{/x} = J \times_I I_{/x}$. 

\begin{rem}
We have $\Delta^{\leq} \to \Delta$ as $I$ and $J$. The homotopy type of $(\Delta^{\leq n}_S)_{/n+k}$, with $k\geq 0$,
is a wedge of a number $N_{n,k}$ of $n-1$-spheres. $N_{n,1} = 1$ and $N_{n,2}= 3$. So the minimum $n$ we can take is 3,
and this gives back the usual theorem of Grothendieck.
\end{rem}
